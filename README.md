...
已有当日资讯 JSON 后，也可以直接执行本地收口与官方国考岗位同步：
...
- `/xingce`：行测模块及本地路线承载页。
- `/job`：展示 Skills 已筛选出的真实符合岗位，支持搜索、筛选、信源跳转及个人跟踪状态。
...
`scan:today` 会先检查或交互创建本地画像，再运行 `scripts/import_national_jobs.py`，从国家公务员局官方招考简章与进入面试人员名单同步能够依据当前画像确认的全部岗位和最低进面分，并核查下一招录年度专题是否发布，最后对当日时政及岗位 JSON 做校验并生成报告。地方事业单位、国企等岗位由 `agents/skills/today-scan/SKILL.md` 按官方原文全渠道追加核验，不会被国考同步覆盖。

该命令不会调用模型。`ensure_profile.py` 仅在本地创建画像；`import_national_jobs.py` 仅访问国家公务员局官方附件并进行确定性筛选；`today_scan.py` 不联网，仅校验 Agent / Skill 已整理的时政与岗位 JSON，并输出 Markdown 与本次扫描清单。前端页面本身始终不发起采集任务。
...
岗位筛选结果会体现学历、专业、身份条件、目标地域等画像推导信息，因此 `content/local/job/`、本地岗位报告与交付归档默认不进入版本控制。公开仓库只保留扫描程序、Schema 与可公开的权威渠道配置。
...
↓
python scripts/import_national_jobs.py 同步国家公务员局官方附件
↓
...
未来 Python 自动化路线可在 `.env` 中配置模型 Token；前端不读取或暴露 Token。公开时政和通用学习内容可随项目提交；本地画像、报名跟踪、岗位筛选结果及含画像推导信息的交付归档由 `.gitignore` 排除，避免个人状态进入仓库。
