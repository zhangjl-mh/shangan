...
PROFILE_PATH = REPO_ROOT / "data" / "user-profile" / "profile.json"
OFFICIAL_DIR = REPO_ROOT / "data" / "jobs" / "sources" / "national-civil-service" / "2026"
LOCAL_JOB_DIR = REPO_ROOT / "data" / "jobs"
...
def main() -> None:
    ...
    domain = {
        ...
        "sources": [
            {
                ...
                "artifact": {
                    "path": str(main_path.relative_to(REPO_ROOT)).replace("\\", "/"),
            ...
            },
            {
                ...
                "checkedAt": generated_at,
                "artifact": {
                    "path": str(supplement_path.relative_to(REPO_ROOT)).replace("\\", "/"),
            ...
            },
            {
                ...
                "checkedAt": generated_at,
                "artifact": {
                    "path": str(interview_path.relative_to(REPO_ROOT)).replace("\\", "/"),
        ...
        "catalog": {
            "path": str(CATALOG_PATH.relative_to(REPO_ROOT)).replace("\\", "/"),
            "rowCount": catalog_count,
            "profileIndependent": True,
        },
        "decisions": {
            "path": str(decision_path.relative_to(REPO_ROOT)).replace("\\", "/"),
